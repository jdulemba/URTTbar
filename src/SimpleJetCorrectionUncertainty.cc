...
float SimpleJetCorrectionUncertainty::uncertaintyBin(unsigned fBin, float fY, bool fDirection) const 
{
  if (fBin >= mParameters->size()) {
    ...
    sserr << "SimpleJetCorrectionUncertainty wrong bin: "<<fBin<<": only "<<mParameters->size()<<" are available";
    throw std::runtime_error(sserr.str());
  }
  const std::vector<float>& p = mParameters->record(fBin).parameters();
  if ((p.size() % 3) != 0) {
    std::stringstream sserr;
    sserr << "SimpleJetCorrectionUncertainty wrong # of parameters: multiple of 3 expected, "<<p.size()<< " got";
    throw std::runtime_error(sserr.str());
  }
