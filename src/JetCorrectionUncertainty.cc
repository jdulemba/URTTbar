  ...
  for(unsigned i=0;i<fNames.size();i++)
    {
      if (fNames[i] == "JetEta")
        ...
        {
          if (!mIsJetPtset)
            throw std::runtime_error("JetCorrectionUncertainty:: jet pt is not set");  
          result.push_back(mJetPt);
        }
      else if (fNames[i] == "JetPhi")
        {
          if (!mIsJetPhiset)
            throw std::runtime_error("JetCorrectionUncertainty:: jet phi is not set");  
          result.push_back(mJetPt);
        }
      else if (fNames[i] == "JetE")
        {
          if (!mIsJetEset)
            throw std::runtime_error("JetCorrectionUncertainty:: jet energy is not set");
          result.push_back(mJetE);
        }
      else if (fNames[i] == "JetEMF")
        {
          if (!mIsJetEMFset)
            throw std::runtime_error("JetCorrectionUncertainty:: jet emf is not set");
          result.push_back(mJetEMF);
        } 
      else if (fNames[i] == "LepPx")
        {
          if (!mIsLepPxset)
            throw std::runtime_error("JetCorrectionUncertainty:: lepton px is not set");  
          result.push_back(mLepPx);
        }
      else if (fNames[i] == "LepPy")
        {
          if (!mIsLepPyset)
            throw std::runtime_error("JetCorrectionUncertainty:: lepton py is not set");  
          result.push_back(mLepPy);
        }
      else if (fNames[i] == "LepPz")
        {
          if (!mIsLepPzset)
            throw std::runtime_error("JetCorrectionUncertainty:: lepton pz is not set");  
          result.push_back(mLepPz);
        }
      else
        throw std::runtime_error("JetCorrectionUncertainty:: unknown parameter "+fNames[i]);
    }     
  return result;      
}
//------------------------------------------------------------------------ 
...
float JetCorrectionUncertainty::getPtRel()
{
  ...
  if (!(lj2 > 0))
    throw std::runtime_error("JetCorrectionUncertainty not positive lepton-jet momentum: ");
