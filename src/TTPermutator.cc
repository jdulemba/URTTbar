...
bool TTPermutator::preselection(vector<IDJet*> jets, TLorentzVector* lepton, IDMet* met, int lc, bool track) {
  ...
  //check b-tagging conditions
	if(IDJet::id_type(cut_tight_b_) == IDJet::IDType::DEEPCSV){
		sort(capped_jets_.begin(), capped_jets_.end(), [](IDJet* A, IDJet* B){return(A->DeepCSVbDisc() > B->DeepCSVbDisc());});
    }
	else if(IDJet::id_type(cut_tight_b_) == IDJet::IDType::DEEPJET){
		sort(capped_jets_.begin(), capped_jets_.end(), [](IDJet* A, IDJet* B){return(A->DeepJetbDisc() > B->DeepJetbDisc());});
    }
