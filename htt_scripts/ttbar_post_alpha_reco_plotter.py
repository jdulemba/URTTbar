'''
Analyzer Plotter macro for 3-jet events after alpha correction applied
'''
...
parser = argparse.ArgumentParser(description='Create plots using files from ttbar_post_alpha_reco.')
    ...
    for kvar in reco_hists.keys():
        ...
        for obj in reco_hists[kvar].keys():
            ...
            for corr_type in reco_hists[kvar][obj].keys():
                ...
                plotter.set_histo_style(hist, xtitle='%s %s' % (corr_type, xlabel), ytitle=defyax, fillstyle=0)
...
def Post_Alpha_Plots( plot ):
    ...
    if plot == 'Partons_Acceptances':
        Parton_Acceptances_Plots( reco_dir , reco_subdir )
