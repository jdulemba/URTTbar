...
class HTTPlotter(Plotter):
    def __init__(self, mode, lumi=None):
        ...
        MC_files = [ fname for fname in files if not ( 'data' in os.path.basename(fname) or 'HtoTT' in os.path.basename(fname) ) ]
        ...
        MC_lumis = [ fname for fname in lumis if not ( 'data' in os.path.basename(fname) or 'HtoTT' in os.path.basename(fname) ) ]
        ...
        #set_trace()
        self.views['ttJets_preselection'] = self.views['ttJetsAll']
    ...
    def create_tt_subsample(self, subdirs, title, color='#9999CC'):
        ...
        shifts[''] = self.create_subsample('ttJetsAll', subdirs, title, color=color)
...
permutations = [
    ...
    #(False, "mass_discriminant", "#lambda_{M}", 1, [6,8,10,12,20], False),
