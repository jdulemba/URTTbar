      ...
      for(string cut : {loose_blabel_, tight_blabel_}) {
        for(string flav : {"_bjet_", "_cjet_", "_ljet_"}) {
          for(string cat : {"all", "pass"}) {
            ...
            histos_[shift][key]= RObject::book<TH2D>(name.c_str(), "btag SF input histograms;p_{T};#eta", 100, 0, 1000, 30, -3, 3);
          }
        }
      }
      ...
      for(auto entry : IDJet::tag_names) {
        for(string flav : {"_bjet_", "_cjet_", "_ljet_"}) {
          for(string cat : {"all", "pass"}) {
            ...
            string key = dir+"/"+name;
            histos_[shift][key] = RObject::book<TH2D>(name.c_str(), "SF Input;p_{T};#eta", 100, 0, 1000, 30, -3, 3);
          }
        }
      }
    }
