    ...
    public:
        permProbComputer(const std::string output_filename):
            ...
            muon_sf_("muon_sf"){
                ...
                Logger::log().debug() << "  isTTJets: " << isTTJets_ << endl;
        ...
        virtual void begin() {
            ...
            for(auto evt_type : evt_types){
                ...
                for(auto shift : systematics_) {
                    ...
                    histos_[shift][evt_type]["wjets_DeepCSV"] = RObject::book<TH2D>("wjets_DeepCSV", "", 100, -1., 1.1, 100, -1., 1.1);
                    histos_[shift][evt_type]["bjets_DeepCSV"] = RObject::book<TH2D>("bjets_DeepCSV", "", 100, -1., 1.1, 100, -1., 1.1);
        ...
        void process_evt(URStreamer &event, systematics::SysShifts shift=systematics::SysShifts::NOSYS) {
            ...
            for(auto test_perm : permutator_.permutations()) {
                ...
                auto b_mM = Minmax(test_perm.BHad()->DeepCSV(), test_perm.BLep()->DeepCSV());
                auto w_mM = Minmax(test_perm.WJa()->DeepCSV(), test_perm.WJb()->DeepCSV());
                plots[perm_status]["wjets_DeepCSV"].fill(pow(w_mM.first, 11), pow(w_mM.second, 11), evt_weight_);
                plots[perm_status]["bjets_DeepCSV"].fill(pow(b_mM.first, 11), pow(b_mM.second, 11), evt_weight_);
                plots[perm_status]["wjets_bDeepCSV_p11"].fill(pow(w_mM.second, 11), evt_weight_);
                plots[perm_status]["wjets_wDeepCSV_p11"].fill(pow(w_mM.first,  11), evt_weight_);
                ...
                plots[perm_status]["wjets_DeepCSV_WP"].fill(wwp_mM.first, wwp_mM.second, evt_weight_);
                plots[perm_status]["bjets_DeepCSV_WP"].fill(bwp_mM.first, bwp_mM.second, evt_weight_);
