...
right_permdisc_shapes = [
   ('3J_mbpjet_vs_maxmjet_correct', 'Max M_{jet}', 'M_{b+jet}', 'A.U.'),
...
wrong_permdisc_shapes = [
   ('3J_mbpjet_vs_maxmjet_wrong', 'Max M_{jet}', 'M_{b+jet}', 'A.U.'),
...
#input_files = glob.glob('ttJetsM0.jet_perm_disc.test.root')
input_files = glob.glob('results/%s/jet_perm_disc/ttJetsM0.root' % jobid)
...
#          right_i +=1
#
#       for shape, xtit, ytit, ztit in wrong_shapes:
#        #  hwrong = wrong_view.Get(path)
##          hwrong = wrong_view.Get(shape)
#          hwrong = Scaled_wrong[wrong_i]
#          print "shape: %s" % shape
#          print "Wrong int: %f" % hwrong.Integral()
#
#          hwrong.name = shape
